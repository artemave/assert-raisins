...
describe('running tests', function() {
  it('works via cli using relative path', function() {
    const stdout = execSync('./bin/ars.mjs ./test/cli/someTest.js', { encoding: 'utf-8' })
    assert.match(stdout, new RegExp('✓ 1/0'))
  })

  it('works via cli using absolute path', function() {
    const stdout = execSync(`./bin/ars.mjs ${path.resolve('./test/cli/someTest.js')}`, { encoding: 'utf-8' })
    assert.match(stdout, new RegExp('✓ 1/0'))
  })
  ...
  it('takes a glob for list of files', function() {
    const stdout = execSync('./bin/ars.mjs "./test/cli/**/*Test.js"', { encoding: 'utf-8' })
  ...
  it('sets exit 1 if test fails', function() {
    try {
      execSync('./bin/ars.mjs ./test/error/failTest.js')
    } catch (err) {
      assert(err.status === 1)
    }
  })
  ...
  it('accepts --only option', function() {
    const stdout = execSync('./bin/ars.mjs --only="also works" "test/cli/**/*Test.js"', { encoding: 'utf-8' })
    assert.match(stdout, new RegExp('✓ 1/0'))
  })

  it('accepts line number', function() {
    const stdout = execSync('./bin/ars.mjs test/cli/one/otherTest.js:9', { encoding: 'utf-8' })
    assert.match(stdout, new RegExp('✓ 1/0'))
  })

  it('runs parallel tests only once', async function() {
    execSync('./bin/ars.mjs "./test/cli/**/*Test.js"', { encoding: 'utf-8' })
