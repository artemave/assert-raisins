...
export function beforeFile(fn, testFile) {
  (testFile || getCurrentSuite()).beforeFile.push(fn)
}
...
export async function run({ only, stdout = process.stdout, suite } = {}) {
  ...
  if (callerModule.getFileName().match(process.argv[1])) {
    ...
    for (const headline in currentSuite) {
      ...
      await Promise.all(testFile.beforeFile.map(fn => fn(registerAfterAllCleanup)))
